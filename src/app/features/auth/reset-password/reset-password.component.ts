...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, PrivacyModalComponent, MatCheckboxModule, MatFormFieldModule, MatInputModule, MatButtonModule],
  template: `
      ...
      <main class="auth-main">
        <div class="form-wrapper">
          ...
          <form [formGroup]="resetForm" (ngSubmit)="onSubmit()" class="login-form">
            ...
            <div class="form-group">
              <mat-checkbox formControlName="privacyAccepted" color="primary" class="consent-check">
                He leído y acepto el <a href="#" (click)="openPrivacyModal($event)" style="color: var(--accent-purple); text-decoration: underline;">Aviso de Privacidad del Sistema</a>.
              </mat-checkbox>
                <div class="error" *ngIf="resetForm.get('privacyAccepted')?.hasError('required')" style="display: block; margin-top: 0.5rem; color: #ef4444;">Debes aceptar el Aviso de Privacidad.</div>
            </div>

            <div *ngIf="errorMessage" class="error-alert">{{ errorMessage }}</div>
            <div *ngIf="successMessage" class="success-alert" style="margin-top: 1rem; padding: 0.75rem; background: rgba(16, 185, 129, 0.1); color: #10b981; border: 1px solid rgba(16, 185, 129, 0.2); border-radius: 8px; font-size: 0.9rem;">{{ successMessage }}</div>
    ...
    <app-privacy-modal [show]="showPrivacyModal" (close)="closePrivacyModal()"></app-privacy-modal>
  `,
